...
class LavaDroplet(private val cubeLocations: List<Vector3>) {
    ...
    private val boundingBox: Pair<Vector3, Vector3> by lazy {
    ...
    private fun isOutsideBoundingBox(vec: Vector3): Boolean {
        val (min, max) = boundingBox
        return vec.x < min.x || vec.y < min.y || vec.z < min.z || vec.x > max.x || vec.y > max.y || vec.z > max.z
    }

    private fun hasPathOut(cubeLocation: Vector3): Boolean {
